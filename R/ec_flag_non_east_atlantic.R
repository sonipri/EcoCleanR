#' flag data points not in east atlantic
#'
...
ec_flag_non_east_atlantic <- function(ocean_names, buffer_distance = 50000, ecodata) {
  ...
  ecodata_sf <- st_sf(geometry = st_sfc())
  # Get ocean shapes
  ...
  bbox <- st_bbox(c(xmin = -40, xmax = 20, ymin = -60, ymax = 60), crs = st_crs(combined_ocean))
  suppressWarnings({
    # Crop the ocean shapes to the bounding box
    cropped_ocean <- st_crop(combined_ocean, bbox)

    # Apply buffer and transform back
    ocean_proj <- st_transform(cropped_ocean, crs = 3395)
    ocean_buffered <- st_buffer(ocean_proj, dist = buffer_distance)
    ocean_buffered <- st_transform(ocean_buffered, crs = 4326)

    # Crop the buffered ocean shapes again with bounding box
    cropped_buffered_ocean <- st_crop(ocean_buffered, bbox)
  })


  valid_ocean <- st_make_valid(cropped_buffered_ocean)

  # Convert ecodata to sf object
  ecodata_sf <- st_as_sf(ecodata, coords = c("decimalLongitude", "decimalLatitude"), crs = 4326)
  ecodata_sf <- st_transform(ecodata_sf, st_crs(valid_ocean))

  # Identify points inside or outside the ocean boundary
  intersect_indices <- st_intersects(ecodata_sf, valid_ocean, sparse = FALSE)
...
# a code to see the map view:

# ggplot() +
#   geom_sf(data = pacific_border_valid, fill = "lightblue", color = "darkblue", alpha = 0.5) +
#   geom_sf(data = ecodata_sf, aes(color = factor(is_pacific)), size = 2) +
#
#   # Customize the plot
#   theme_minimal() +
#   ggtitle("Species Occurrences Inside and Outside Pacific Ocean Border") +
#   scale_color_manual(values = c("1" = "red", "0" = "green"), labels = c("Good", "bad")) +
