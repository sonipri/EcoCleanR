...
In this tutorial, we demonstrate the step-by-step process of downloading
data from various sources such as GBIF, OBIS, and iDigBio using existing
R packages, as well as from InvertEbase via a local CSV file. This
process includes merging all data files and standardizing their formats
to make them compatible for integration.
...
Given attributes in the list can be changed/added based on the
requirement
...
This step uses function `occ_data` of rgbif package to extract data from
GBIF.
...
This step uses `occurrence` function of robis package to extract data
from OBIS.
...
This step uses `idig_search_records` of ridigbio package to extract data
from IDIGBIO.
...
This local file “example_sp_invertebase” is a manual downloaded file
from InvertEbase for Mexacanthina lugubris. See the
example_sp_invertEbase dataset for its attributes and DwC format.
...
`ec_db_merge` function in the EcoCleanR package helps merge data from
all sources, provided that each source has the same attribute names and
number of columns. It also filters the data based on the specified type
(e.g., modern or fossil) and removes records marked as ‘absent’
occurrenceStatus.
