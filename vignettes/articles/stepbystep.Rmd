...
inst_counts <- gbif.occ %>%
  group_by(institutionCode) %>%
  summarise(record_count = n(), .groups = "drop")

### optional to perform..
gbif.occ_temp <- gbif.occ %>%
  filter(is.na(institutionCode) | institutionCode != "NRM")

# remove unwanted insititution codes which may not have valid records
...
sym.occ <- example_sp_invertebase # this file is currently loaded in package "/data" folder. We can either make a file manually of use rsymbiota for automation.
...
ecodata_cl <- ec_filter_by_uncertainty(ecodata_cl, uncertainty_col = "coordinateUncertaintyInMeters", percentile = 0.96, ask = TRUE)
...
list_outlier <- list()
list_outlier <- ec_flag_outlier(ecodata_unique, env_layers, itr = 100, k = 3, geo_quantile = 0.99, maha_quantile = 0.99)

ec_plot_distance(list_outlier[["result_list"]], geo_quantile = 0.99, maha_quantile = 0.99)
...
ecodata_cl <- ecodata_cl %>%
  left_join(ecodata_unique[, c("decimalLatitude", "decimalLongitude", "flag_outliers", "temperature_mean", "temperature_max", "temperature_min")],
