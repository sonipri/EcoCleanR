...
ecodata <- ec_merge_corrected_coordinates(ecodata_corrected, ecodata, catalog = "cleaned_catalog")
str(ecodata[,1:3]) 

### Plot the map to visualize the datapoints
ec_geographic_map(ecodata)###plot the map
```
...
```{r}
ecodata_cl <- ec_filter_by_uncertainty(ecodata, uncertainty_col = "coordinateUncertaintyInMeters", percentile = 0.95, ask = FALSE)
str(ecodata_cl[, 1:3])
### plot the map
...
ecodata_unique <- ec_extract_env_layers(ecodata_unique, env_layers = env_layers)
...
summary_table <- ec_var_summary(ecodata_cleaned, env_layers = env_layers)
head(summary_table)

ec_plot_var_range(ecodata_with_outliers, summary_df = summary_table, env_layers = env_layers)
```
