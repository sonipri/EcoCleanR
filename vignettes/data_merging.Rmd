...
Since the rsymbiota package has been archived and is no longer functional, a sample file "example_sp_invertebase" downloaded from InvertEbase for Mexacanthina lugubris is provided instead. See the example_sp_invertEbase dataset for its attributes and format.
```{r}
...
`ec_db_merge` unction in the EcoCleanR package helps merge data from all sources, provided that each source has the same attribute names and number of columns. It also filters the data based on the specified type (e.g., modern or fossil) and removes records marked as 'absent' occurrenceStatus.
```{r}
Mixdb.occ <- ec_db_merge(datatype = "modern", gbif.occ_temp, obis.occ_temp, idig.occ_temp, sym.occ)

str(Mixdb.occ[, 1:3])
ec_geographic_map(Mixdb.occ) #display records those has coordinate values
```
